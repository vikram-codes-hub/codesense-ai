...
const Section = ({ title, icon: Icon, accent = false, children }) => (
  <div style={{
    ...
    <div style={{
      padding: '16px 24px',
      borderBottom: `1px solid ${accent ? 'rgba(239,68,68,0.2)' : 'var(--border)'}`,
...
const Field = ({ label, hint, children }) => (
  <div style={{ marginBottom: 18 }}>
    ...
    {children}
    {hint && (
      <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 5 }}>{hint}</p>
    )}
  </div>
)
      ...
      <Section title="Profile" icon={User}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            <img
              src={user?.avatar || `https://ui-avatars.com/api/?name=${user?.name || 'U'}&background=6366f1&color=fff&size=80`}
              alt="avatar"
              style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <Field label="Confirm New Password">
            <input
              ...
              style={{
                borderColor: passwords.confirm
                  ? passwords.newPass === passwords.confirm ? 'var(--grade-a)' : 'var(--critical)'
                  : undefined
              }}
            />
            {passwords.confirm && (
              <p style={{
                ...
                color: passwords.newPass === passwords.confirm ? 'var(--grade-a)' : 'var(--critical)',
              }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>
                {user?.githubId ? `@${user.githubId}` : 'Not connected'}
              </p>
              <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '2px 0 0' }}>
                {user?.githubId ? 'GitHub account connected ✓' : 'Connect to enable PR webhook reviews'}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button
              ...
              onClick={() => toast.success(user?.githubId ? 'Disconnected!' : 'Connect GitHub in production!')}
            >
              {user?.githubId ? 'Disconnect' : 'Connect GitHub'}
            </button>
          </div>
        </div>
      </Section>
        ...
        {[
          ...
          <div key={key} style={{
            ...
            >
              <div style={{
                position: 'absolute',
                top: 3, left: notifications[key] ? 22 : 3,
