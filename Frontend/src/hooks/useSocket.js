...
export const useSocket = (reviewId = null) => {
  ...
  useEffect(() => {
    ...
    socket.on('review:queued', (data) => {
      addLiveUpdate({ ...data, event: 'review:queued', time: new Date().toLocaleTimeString() })
    })

    socket.on('review:started', (data) => {
      addLiveUpdate({ ...data, event: 'review:started', time: new Date().toLocaleTimeString() })
      updateReviewStatus(data.reviewId, 'processing')
    })
    ...
    socket.on('review:complete', (data) => {
      addLiveUpdate({ ...data, event: 'review:complete', time: new Date().toLocaleTimeString() })
    ...
    socket.on('review:failed', (data) => {
      addLiveUpdate({ ...data, event: 'review:failed', time: new Date().toLocaleTimeString() })
      updateReviewStatus(data.reviewId, 'failed')
    })

    socket.on('review:posted', (data) => {
      addLiveUpdate({ ...data, event: 'review:posted', time: new Date().toLocaleTimeString() })
    })
