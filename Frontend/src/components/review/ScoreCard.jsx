...
const getScoreGrade = (score) => {
  ...
  if (score >= 60) return { grade: 'C', color: '#eab308', label: 'Needs Work'      }
  if (score >= 40) return { grade: 'D', color: '#f97316', label: 'Poor'            }
  return               { grade: 'F', color: '#ef4444', label: 'Critical Issues' }
}
...
export default function ScoreCard({ review }) {
  if (!review?.overallScore) return null
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
            {review.totalIssues ?? 0} issues found
          </div>
        </div>
      </div>
        ...
        {categories.map(({ key, label, icon: Icon, weight, color }) => (
          <div key={key}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color }}>{review[key] ?? '—'}</span>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 16 }}>
        {[
          ...
          <div key={label} style={{
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color }}>{count ?? 0}</span>
          </div>
        ))}
      </div>
    </div>
